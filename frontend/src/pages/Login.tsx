...
const Login = () => {
  ...
  <div className={`group relative rounded-2xl shadow-2xl border p-6 sm:p-8 transform-gpu transition-all duration-300 bg-[rgba(255,255,255,0.06)] dark:bg-[rgba(6,10,15,0.24)] hover:scale-[1.02] hover:shadow-2xl shadow-inner ${exiting ? 'opacity-0 translate-y-6 scale-95' : 'opacity-100 translate-y-0 scale-100'}`} style={{ backdropFilter: 'blur(22px)', WebkitBackdropFilter: 'blur(22px)', borderColor: 'rgba(255,255,255,0.04)'}}>
          <header className="text-center mb-6">
            ...
            <h1 className="mt-3 text-xl sm:text-2xl font-extrabold text-text dark:text-white">Sign in to SafePathAI</h1>
            <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">Enter your credentials to continue</p>
          </header>
          ...
          <form onSubmit={handleLogin} className="space-y-5 md:space-y-6">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 sm:px-4 py-3 rounded-lg border transition-shadow duration-150 outline-none bg-[rgba(255,255,255,0.08)] placeholder-gray-400 text-gray-900 dark:text-gray-100 dark:bg-[rgba(255,255,255,0.02)] select-text hover:shadow-sm text-sm sm:text-base md:text-base ${errors.identifier ? 'border-red-400 ring-1 ring-red-200' : 'border-transparent focus:ring-2 focus:ring-primary/60'}`}
              />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 md:py-4 rounded-lg text-white font-semibold bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 transform transition-all disabled:opacity-60 text-sm sm:text-base md:text-base"
            >
          ...
          <div className="mt-4 text-center text-sm text-gray-600 dark:text-gray-300">
            ...
            <button type="button" onClick={(e) => goToSignup(e)} disabled={exiting} className="text-violet-600 hover:underline dark:text-violet-400 font-medium">Sign up here</button>
          </div>
        </div>

        <p className="mt-6 text-center text-xs text-gray-500 dark:text-gray-400">By continuing, you agree to our Terms of Service and Privacy Policy.</p>
      </div>
    </div>
  );
};
